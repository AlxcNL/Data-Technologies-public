...
Because JSON is a format on it's own, we need to do use specific functions inside a SELECT query to grab information from the JSON field. <br> The `->` operator extracts a JSON object or array, so we can use it inside the query. 
